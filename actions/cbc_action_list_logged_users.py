...
class ListLoggedUsersAction(BaseAction):
    ...
    def _list_logged_users(self):
        ...
        if os == "WINDOWS":
            results = live_query.run(WINDOWS_SQL_QUERY, device_id, run_name="Splunk SOAR CBC App - List Logged in Users (Windows)")
        elif (os == "MAC") or (os == "LINUX"):
            results = live_query.run(LINUX_MAC_SQL_QUERY, device_id, run_name="Splunk SOAR CBC App - List Logged in Users (MacOS/Linux)")
        else:
